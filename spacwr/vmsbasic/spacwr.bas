%TITLE "SPACWR (STARTREK) FOR VMS BASIC"
...
1710 GOSUB 5610
172 0PRINT" DAMAGED" \ PRINT \ GOTO 1810
...
1940 IF S1<1 or S1>=9 GOTO OR S2<1 OR S2>=9 THEN 2170
1950 S8=S1*24+S2*3-26 \ IF S8>72 THEN 1990
1970 IF MID$(Q$,S8,3)="   " THEN 2070
1980 GOTO 2030
1990 IF S8>144 THEN 2020
2000 IF MID$(R$,S8-72,S8-70)="   " THEN 2070
2010 GOTO 2030
2020 IF MID$(S$,S8-144,S8-142)="   " THEN
...
2460 NEXT J
2470 P1$=": ### : ### : ### :"
2471 PRINT USING P1$,(N1),N(2),N(3)
2480 PRINT"-------------------"
2490 NEXT I
2500 GOTO 1270
...
2960 X=X+X1 \ Y=Y+X2
2980 IF X<1 OR X>=9 OR IF Y<1 OR IF Y>=9 THEN 3420
2990 PRINT "               ";X;",";Y
3010 A$="   "
3040 GOSUB 5680
...
3830 FOR I=1 TO 3 \ IF K(I,3)<=0 THEN 3900
...
4500 O8$=02$+"       PHOTON TORPEDOES  ###"
4505 PRINT USING 08$,MID$(S$,1,3),MID$(S$,4,3),MID$(S$,7,3),MID$(S$,10,3),MID$(S$,13,3),MID$(S$,16,3),MID$(S$,19,3),MID$(S$,22,3),P
4510 O9$=02$+"        SHIELDS       ######"
4515 PRINT USING O8$,MID$(S$,25,3),MID$(S$,28,3),MID$(S$,31,3),MID$(S$,34,3),MID$(S$,37,3),MID$(S$,40,3),MID$(S$,43,3),MID$(S$,46,3),S
4520 PRINT USING O1$
4530 RETURN
...
5821 PRINT "THE GALAXY IS DIVIDED INTO AN 8,8 QUADRANT GRID"
5822 PRINT "WHICH IN TURN IS DIVIDED INTO AN 8X8 SECTOR GRID."
...
5920 PRINT "  HALF WAY BETWEEN 1 AND 2.                  /V\"
5930 PRINT "                                            / V \"
...
6030 PRINT "  STARS, KLINGONS, STARBASES AND THE THE ENTERPRISE. ALONG"
6040 PRINT "  WITH OTHER PERTINENT INFORMATION"
6045 PRINT \ PRINT "COMMAND 2 = LONG RANGE SENSOR SCAN"
6060 PRINT "  SHOWS CONDITIONS IN SPACE FOR A QUADANT ON EACH SIDE"
...
6100 PRINT "  BASES,  THE HUNDREDS DIGIT IS THE NUMBER OF KINGONS."
6110 PRINT \ PRINT "COMMAND 3 = PHASER CONTROL"
6120 PRINT "  ALLOWS YOU TO DESTROY THEKLINGONS BY HITTING HIM WITH"
...
6180 PRINT "  IF YOU HIT THE KLINGONM HE IS DESTROYED AND CANNOT FIRE"
6190 PRINT "  BACK AT YOU.  IF YOU MISS, YOU ARE SUBJECT TO HIS"
6200 PRINT "  PHASER FIRE."
6210 PRINT \ PRINT "  NOTE: THE LIBRAY COMPUTER (COMMAND 7) HAS AN OPTION"
...
6320 PRINT "    OPTION 0 = CUMULATIVE GALACTIC RECORS"
...
6490 PRINT "      GIVES TRAJECTORY AND DISTACE BETWEEN THE"
